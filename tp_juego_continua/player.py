...
class Jugador:
    ...
    def __init__(self, coord_x, coord_y, frame_rate = 100, speed_walk = 6, speed_run = 12, gravity = 5, jump = 60):
    ...
    def draw(self, screen):
        screen.blit(self.__actual_animation[self.__initial_frame], self.__rect.topleft)
    ...
    def jump(self, jumping=True):
        if jumping and not self.__is_jumping and self.on_ground:
            self.__set_y_animations_preset()
            self.__jump_start_y = self.__rect.y
            self.__actual_animation = self.__jump_r if self.__is_looking_right else self.__jump_l
    ...
    def do_movement(self, delta_ms):
        ...
        if self.__player_move_time >= self.__frame_rate:
            ...
            new_X = self.__rect.x +self.__move_x
            ...
            if self.__is_jumping:
                ...
                if self.__rect.y >= self.__jump_start_y:
                    self.__is_jumping = False
                    self.__rect.y = self.__jump_start_y
        elif self.__rect.y < 300:
